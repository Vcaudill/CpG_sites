...
#ancestor
# data:  log(df2$CpG.Cost.A) and df2$cg.rho
# S = 19722, p-value = 4.121e-05
# alternative hypothesis: true rho is not equal to 0
# sample estimates:
#     rho 
# -0.5980877 
...
#ancestor
# data:  df.plot$value and df.plot$CG.rho
# S = 153150, p-value = 5.769e-08
# alternative hypothesis: true rho is not equal to 0
# sample estimates:
#     rho 
# -0.5505676 
...
ggplot(df.plot, aes(x=CG.rho, y=value, color=variable))+
    ...
    scale_color_manual(values=colors[c(1,5)], labels=c("A->G","T->C"))+
    theme_bw()+
